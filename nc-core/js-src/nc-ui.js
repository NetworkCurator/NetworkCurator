...
nc.ui.DropdownObjectList = function (atype, aa, aval, withdeprecated) {
    ...
    for (var i in aa) {
        ...
        if (iinclude) {
            html += '<li' + iclass + '><a class_id="' + aa[i].id + '" class_name="'
                    + aa[i].val + '" href="#">' + aa[i].label + '</a></li>';
        }
    }
    html += '</ul>';
...
/**
 * 
 * @param nodetypes arrays with objects, each object holding id, label, val
 * @param linktypes
 */
...
/**
 * Create a button with a dropdown list for saving graphs
 */
